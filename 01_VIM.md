...
- `vi + <fileName>` : file을 열고 맨 마지막 줄에 커서 위치
- `vi +n <fileName>` : file을 열고 n번째 줄에 커서 위치
- `vi -r <fileName>` : 손상된 파일 회복

<b>입력모드 전환</b>
- `i` : 현재 위치에서 insert
- `I` : 커서가 위치한 줄 맨 앞에서 insert
- `a` : 커서가 위치한 다음 칸에서 insert
- `A` : 커서가 위치한 줄의 맨 마지막에서 insert
